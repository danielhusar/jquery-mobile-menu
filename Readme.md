...
### Challenge
If you have fixed navbar like yeoman.io, and in this navbar is button that trigger mobile menu to appear, regulary you want to have this menu to be fixed from top so no matter where the user have page scrolled, it always appear above the content. This solution is not very good if the menu is lonnger than the page, as its very hard for user to scroll the menu. This plugin just remember the user position at the page so menu can have absolute position, and showing/hiding restore the page scrolling position.
